							  ...
							  LPTSTR  lpszFullName)
{
	...
	//
	// If the old SID is in the group, we have to write
	// the updated owner to disk
	//
...
void UpdateFileSID( HWND hDlg, PTCHAR PathName )
{
	...
	//
	searchName = (TCHAR *) malloc( TLEN( PathName ) + TLEN( _T("\\*.*")));
	...
	do {
		...
		if( wcscmp( foundFile.cFileName, L"." ) &&
			...
			//
			subName = (TCHAR *) malloc( TLEN( PathName ) + TLEN( _T("\\")) + TLEN( foundFile.cFileName ));
