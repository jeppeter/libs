	...
	, m_bValidVMsWereFound(false)
{
	...
	if (m_Params.ToolsPath.empty())
	{
		...
		toolsPath = toolsPath + (FilePath)_T("Debugging Tools for Windows");
	}
	else
		toolsPath = m_Params.ToolsPath;
	if (File::Exists(toolsPath + (FilePath)_T("windbg.exe")) && File::Exists(toolsPath + (FilePath)_T("kd.exe")))
...
void CMainDlg::RunDebugger(unsigned entryIndex)
{
	...
	if (proc.PipeName.empty())
		return;
	FilePath debugger(_T(""));
	TCHAR tszCmdLine[MAX_PATH * 2] = {0,};
	if (m_Params.DebuggerType < 2)
	{
		debugger = m_DbgToolsPath + (FilePath)(m_Params.DebuggerType ? _T("windbg.exe") : _T("kd.exe"));
	...
	else
	{
		...
		FindAndReplace(cmdLine, _T("$(toolspath)"), m_DbgToolsPath.ToString());
	...
	CreateProcess(debugger.ToString().empty() ? NULL : debugger.c_str(), tszCmdLine, NULL, NULL, FALSE, CREATE_NEW_PROCESS_GROUP, NULL, m_DbgToolsPath.c_str(), &startupInfo, &processInfo);
...
{
	bool b = !m_DbgToolsPath.ToString().empty();
...
LRESULT CMainDlg::OnBnClickedDbgpath(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	FilePath fileName;
	if (!m_DbgToolsPath.ToString().empty())
	{
		fileName = m_DbgToolsPath + FilePath(_T("windbg.exe"));
	...
	if (dlg.DoModal() == IDOK)
	{
		FilePath dbgPath = dlg.m_szFileName;
		dbgPath = dbgPath.GetParentPath();
		if (File::Exists(dbgPath + FilePath(_T("windbg.exe"))))
		{
			m_DbgToolsPath = dbgPath;
			m_Params.ToolsPath = m_DbgToolsPath.ToString();
			SaveParamsToRegistry();
		}
		OnDebuggerPathChanged();
	}
...
void CMainDlg::DisplayStats(PatchedProcess &proc)
{
	...
	int i = 0;
	m_StatsView.SetItem(i++, 1, LVIF_TEXT, Network::RateCalculator::FormatByteCount(proc.pStatus->BytesSent).c_str(), 0, 0, 0, 0);
	m_StatsView.SetItem(i++, 1, LVIF_TEXT, Network::RateCalculator::FormatByteCount(proc.pStatus->BytesReceived).c_str(), 0, 0, 0, 0);
	...
	m_StatsView.SetItem(i++, 1, LVIF_TEXT, tsz, 0, 0, 0, 0);
	_sntprintf_s(tsz, __countof(tsz), _TRUNCATE, _T("%s/s"), Network::RateCalculator::FormatByteCount(proc.PacketSendRate).c_str());
	m_StatsView.SetItem(i++, 1, LVIF_TEXT, tsz, 0, 0, 0, 0);
	_sntprintf_s(tsz, __countof(tsz), _TRUNCATE, _T("%s/s"), Network::RateCalculator::FormatByteCount(proc.PacketRecvRate).c_str());
	m_StatsView.SetItem(i++, 1, LVIF_TEXT, tsz, 0, 0, 0, 0);

	_sntprintf_s(tsz, __countof(tsz), _TRUNCATE, _T("%d"), proc.pStatus->ResyncCount);
	m_StatsView.SetItem(i++, 1, LVIF_TEXT, tsz, 0, 0, 0, 0);

	_sntprintf_s(tsz, __countof(tsz), _TRUNCATE, _T("%s/s"), Network::RateCalculator::FormatByteCount(proc.RecvRate).c_str());
	m_StatsView.SetItem(i++, 1, LVIF_TEXT, tsz, 0, 0, 0, 0);
	_sntprintf_s(tsz, __countof(tsz), _TRUNCATE, _T("%s/s"), Network::RateCalculator::FormatByteCount(proc.SendRate).c_str());
	m_StatsView.SetItem(i++, 1, LVIF_TEXT, tsz, 0, 0, 0, 0);
	_sntprintf_s(tsz, __countof(tsz), _TRUNCATE, _T("%s/s"), Network::RateCalculator::FormatByteCount(proc.MaxRecvRate).c_str());
	m_StatsView.SetItem(i++, 1, LVIF_TEXT, tsz, 0, 0, 0, 0);
	_sntprintf_s(tsz, __countof(tsz), _TRUNCATE, _T("%s/s"), Network::RateCalculator::FormatByteCount(proc.MaxSendRate).c_str());
	m_StatsView.SetItem(i++, 1, LVIF_TEXT, tsz, 0, 0, 0, 0);
