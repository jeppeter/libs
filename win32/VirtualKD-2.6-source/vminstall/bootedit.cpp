...
#define BOOT_BAK_FILE_NAME	_T("c:\\boot.vkd")
	...
	public:
		...
		{
			ManagedPointer<TextANSIFileReader> pBootIni = new TextANSIFileReader(new OSFile(BOOT_INI_FILE_NAME, FastFileFlags::OpenReadOnly));
			if (!pBootIni->Valid())
				return;
		...
		virtual ActionStatus FinishEditing()
		{
			...
			ActionStatus st;
			ManagedPointer<TextANSIFileReader> pBootIni = new TextANSIFileReader(new OSFile(BOOT_INI_FILE_NAME, FastFileFlags::OpenReadOnly, &st));
			if (!pBootIni->Valid())
			{
				if (!st.Successful())
					return st;
			...
			File newBootIni(tempFileName, FastFileFlags::CreateOrTruncateRW, &st);
			if (!st.Successful())
				return st;
			if (!newBootIni.Valid())
				return MAKE_STATUS(UnknownError);
