...
ActionStatus TryLaunchVVMON(const FilePath &exePath, bool WaitForPipeAvailable = true)
{
...
ULONGLONG Call64BitKDCLIENT(KDClientCommand cmd, unsigned PID, LPWSTR lpStringResult, size_t StringSize)
{
	...
	for (;;)
	{
		...
		GetModuleFileName(g_hThisDll, tszModule, __countof(tszModule));
		FilePath vmmonExe = FilePath(tszModule).Parent() + FilePath(WOW64APIProvider::sIsWow64Process() ? _T("vmmon64.exe") : _T("vmmon.exe"));
